...
JL_DLLEXPORT jl_value_t *jl_atomics_new_bits(jl_value_t *dt, const char *data)
{
...
JL_DLLEXPORT void jl_atomics_store_bits(char *dst, const jl_value_t *src, int nb)
{
...
JL_DLLEXPORT jl_value_t *jl_atomics_swap_bits(jl_value_t *dt, char *dst, const jl_value_t *src, int nb)
{
    // dst must have the required alignment for an atomic of the given size
    jl_ptls_t ptls = jl_get_ptls_states();
    assert(jl_is_datatype(dt));
...
JL_DLLEXPORT jl_value_t *jl_atomics_pointerset(jl_value_t *p, jl_value_t *x, jl_value_t *order)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        jl_atomics_store_bits(pp, x, nb);
    }
    return p;
}

JL_DLLEXPORT jl_value_t *jl_atomics_pointerswap(jl_value_t *p, jl_value_t *x, jl_value_t *order)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        y = jl_atomics_swap_bits(ety, pp, x, nb);
    }
    return y;
}

JL_DLLEXPORT jl_value_t *jl_atomics_pointermodify(jl_value_t *p, jl_value_t *f, jl_value_t *x, jl_value_t *order)
{
    jl_value_t **args;
    JL_GC_PUSHARGS(args, 2);
    while (1) {
        jl_value_t *expected = jl_atomics_pointerref(p, order);
        args[0] = x;
        args[1] = expected;
        jl_value_t *y = jl_apply_generic(f, args, 2);
        args[0] = y;
        if (jl_atomics_pointercmpswap(p, y, expected, order, order) == jl_true) {
            JL_GC_POP();
            return expected;
        }
        jl_gc_safepoint();
...
JL_DLLEXPORT jl_value_t *jl_atomics_pointercmpswap(jl_value_t *p, jl_value_t *x, jl_value_t *expected, jl_value_t *success_order, jl_value_t *failure_order)
{
